...
// must open a themed menu (NOT instantly toggle mute); "Resolve thread" removes
...
(async () => {
  ...
  if (!items.some((t) => /resolve/i.test(t))) fail('no Resolve item in menu: ' + JSON.stringify(items));
  ...
  // reopen and click Resolve -> leaf disappears from the tree
  await page.mouse.click(box.x + box.width / 2, box.y + box.height / 2, { button: 'right' });
  await page.waitForSelector('.context-menu', { timeout: 3000 });
  const resolveIdx = (await page.$$eval('.context-menu .ctx-item', (els) => els.map((e) => e.textContent))).findIndex((t) => /resolve/i.test(t));
  const btns = await page.$$('.context-menu .ctx-item');
  await btns[resolveIdx].click();
  await page.waitForFunction(() => !document.querySelector('.thread-leaf'), { timeout: 4000 }).catch(() => fail('leaf still present after Resolve'));
