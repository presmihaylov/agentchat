...
(async () => {
  ...
  // one sidebar type scale (task 20, Maya msg 6726d1bf): 11px caps headings, 14px rows; the profile name is 13px/500 since the compact footer (dec00282)
  ...
  assert(scale.header === '15px/600' && scale.h3 === '11px/600' && scale.row === '14px/400' && scale.me === '13px/500', 'sidebar scale: ' + JSON.stringify(scale));
  ...
  assert(wsRows.map((r) => r.text).join('|') === 'mail:Invite member|log-in:Join with invite link|bell-off:Mute workspace|settings:Settings', 'ws menu rows: ' + JSON.stringify(wsRows));
